...
<form action="del_kontak.php"method="post">
   ...
   // iki tambahan'e bro!
    ...
    switch ($id_admin) {
        case '2':
            $query = "SELECT * FROM tb_kontak_all WHERE status_kontak = 'belum dihubungi' AND kota_kontak IN ('surabaya', 'sidoarjo', 'gersik', 'lamongan', 'mojokerto', 'jombang', 'nganjuk', 'madiun', 'ngawi', 'ponorogo', 'pacitan', 'trenggalek', 'tulungagung', 'blitar')";
            break;
        case '3':
            ...
            break;
        case '4':
            $query = "SELECT * FROM tb_kontak_all WHERE status_kontak = 'belum dihubungi' AND kota_kontak IN ('cilacap', 'banyumas', 'brebes', 'tegal', 'purbalingga', 'batang', 'banjarnegara', 'kebumen', 'wonosobo', 'purworejo', 'temanggung', 'magelang', 'kendal', 'yogyakarta', 'ungaran', 'salatiga', 'boyolali', 'klaten', 'surakarta', 'sukorejo', 'karanganyar', 'wonogiri', 'sragen', 'grobongan', 'demak', 'semarang')";
            break;
        case '5':
            $query = "SELECT * FROM tb_kontak_all WHERE status_kontak = 'belum dihubungi' AND kota_kontak IN ('jepara', 'kudus', 'pati', 'rembang', 'blora')";
            break;
        case '6':
            $query = "SELECT * FROM tb_kontak_all WHERE status_kontak = 'belum dihubungi' AND kota_kontak IN ('cirebon')";
            break;
        default:
